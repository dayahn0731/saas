        ...
        <div>
            <div role="alert" className={flag ? "xl:w-5/12 mx-auto sm:mx-0 sm:w-6/12 md:w-3/5 w-11/12 bg-white shadow-lg rounded flex sm:flex-row flex-col pr-4 absolute left-0 sm:left-auto right-0 dark:bg-gray-800 sm:top-0 sm:mr-6 mt-16 sm:mt-6 mb-6 sm:mb-0 transition duration-150 ease-in-out translate-show z-20" : "translate-hide"}>
                ...
                <div class="flex flex-col justify-center pl-4 sm:w-9/12 sm:pt-6 pb-6 pt-2">
                    <p class="text-lg text-gray-800 dark:text-gray-100 font-semibold pb-1">{props.header}</p>
                    <p class="text-sm text-gray-600 dark:text-gray-400 font-normal">{props.message}</p>
                    <div className="flex">
                        <span className="text-sm text-gray-600 dark:text-gray-400 cursor-pointer" onClick={() => props.dismiss()}>
                            Dismiss
                        </span>
                    </div>
                </div>
